...
const cacheTitle = `quickdrop-cache-${cacheVersion}`;
...
const evaluateRequestData = function (request) {
    return new Promise(async (resolve) => {
        ...
        const quickDropUrl = request.url;
        ...
        if (files && files.length > 0) {
            ...
            const DBOpenRequest = indexedDB.open('quickdrop_store');
            DBOpenRequest.onsuccess = e => {
                ...
                for (let i = 0; i < fileObjects.length; i++) {
                    ...
                    objectStoreRequest.onsuccess = _ => {
                        if (i === fileObjects.length - 1) resolve(quickDropUrl + '?share_target=files');
                    }
                }
            }
            DBOpenRequest.onerror = _ => {
                resolve(quickDropUrl);
            }
        }
        else {
            ...
            resolve(quickDropUrl + urlArgument);
        }
    });
}
